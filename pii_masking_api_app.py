...
try:
    ...
    def tokenize(text: str) -> List[str]:
        return [m.surface() for m in _tokenizer_obj.tokenize(text, sudachi_tokenizer.Tokenizer.SplitMode.C)]
...
async def mask_endpoint(req: MaskRequest):
    ...
    resp = {"masked_text": masked, "entities": entities, "risk_score": risk_score, "cached": False}
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("pii_masking_api_app:app", host="0.0.0.0", port=int(os.getenv("PORT", 8080)))
